<div class="tab-pane fade show active" id="pills-info" role="tabpanel" aria-labelledby="pills-info-tab">
    <form method="POST" action="{{ route('product.update', ['product' => $info->product_id]) }}" class="forms-sample" enctype="multipart/form-data">
        ...
        <div class="form-group" id="category-{{ $loop->iteration }}">
            <label for="category_alt-{{ $loop->iteration }}" class="align-middle">Category Alt</label>
            <button type="button" class="btn btn-xs btn-rounded btn-gradient-dark ml-3 mb-2" onclick="removeCategory('{{ $loop->iteration }}')">Remove Category</button>
            {!! Form::select("category_alt[]", $categories, $row->category_id, ["class" => "form-control", "id" => "category_alt-".$loop->iteration]) !!}
        </div>
        <div id="subcat-{{ $loop->iteration }}" class="mb-3">
            <label>Sub Category Alt</label>
            <span id="subcat_alt-{{ $loop->iteration }}">
...
<script>
    ...
    $(document).ready(function() {
        ...
        $(document).on('change', '[id^=category_alt-]', function() {
            id = this.id;
